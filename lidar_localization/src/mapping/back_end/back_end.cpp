...
bool BackEnd::MaybeNewKeyFrame(const CloudData &cloud_data, const PoseData &laser_odom, const PoseData &gnss_odom)
{
    ...
    if (fabs(laser_odom.pose(0, 3) - last_key_pose(0, 3)) + fabs(laser_odom.pose(1, 3) - last_key_pose(1, 3)) + fabs(laser_odom.pose(2, 3) - last_key_pose(2, 3)) > key_frame_distance_)
    {
        has_new_key_frame_ = true;
        last_key_pose = laser_odom.pose;
    }
...
bool BackEnd::AddNodeAndEdge(const PoseData &gnss_data)
{
    ...
    std::cout << "@test node_num: " << node_num << std::endl;
    if (node_num > 1)
    {
